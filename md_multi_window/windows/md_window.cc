...
MdWindow::MdWindow(std::string id, HWND handle) : id_(id), window_handle_(handle), scale_factor_(1) {
...
MdWindow::MdWindow(
    std::string id,
    std::string args
) : id_(id), window_handle_(nullptr), scale_factor_(1) {
...
// static
MdWindow *MdWindow::GetThisFromHandle(HWND window) noexcept {
  return reinterpret_cast<MdWindow *>(
      GetWindowLongPtr(window, GWLP_USERDATA));
}

// static
LRESULT CALLBACK MdWindow::WndProc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) {
  if (message == WM_NCCREATE) {
    auto window_struct = reinterpret_cast<CREATESTRUCT *>(lparam);
    SetWindowLongPtr(window, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(window_struct->lpCreateParams));

    auto that = static_cast<MdWindow *>(window_struct->lpCreateParams);
    EnableFullDpiSupportIfAvailable(window);
    that->window_handle_ = window;
  } else if (MdWindow *that = GetThisFromHandle(window)) {
...
LRESULT MdWindow::MessageHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
